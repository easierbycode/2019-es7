...
import { parseSectionTable, validateSectionTable, isGameSave, TABLE_SIZE } from "../lib/payload-table.js";

const FIX = path.join(path.dirname(fileURLToPath(import.meta.url)), "..", "fixtures");
const payloadOf = (name) => {
    const [save] = bup.parse(deinterleave(fs.readFileSync(path.join(FIX, name))));
    assert.ok(isGameSave(save), `${name} should contain a game save`);
...
for (const [fixture, golden] of Object.entries(GOLDEN)) {
    test(`section table of ${fixture} parses and validates (checksums prove reassembly)`, () => {
        const payload = payloadOf(fixture);
        const table = parseSectionTable(payload); // validates by default
        assert.strictEqual(table.checksumTotal, golden.checksumTotal);
        assert.strictEqual(table.tableAddr, golden.tableAddr);
        assert.strictEqual(table.endAddr, golden.endAddr);
        ...
        assert.strictEqual(total, payload.length);
    });
}

test("validation catches a corrupted payload byte", () => {
    ...
    assert.throws(() => validateSectionTable(payload, table), /section 5 checksum mismatch/);
});

test("section table rejects a too-small payload", () => {
    assert.throws(() => parseSectionTable(new Uint8Array(16)), /too small/);
});

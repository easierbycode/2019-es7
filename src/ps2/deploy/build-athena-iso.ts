...
//
// Vendored from cmg0711/scripts/build-athena-iso.ts, hardened for build.sh:
// skips a pre-existing ATHA_000.01 boot stub and normalizes Windows path
// separators so `build.sh --iso` can run it on the build/ directory directly.
//
...
//
// Usage:
//   deno run --allow-read --allow-write build-athena-iso.ts <appDir> [outIso] [bootElfName]
//     appDir      folder with the athena ELF + athena.ini + scripts + assets
//     outIso      output path (default: ./<appDir-name>.iso in the current dir)
//     bootElfName ELF to boot (default: athena.elf, else the first *.elf found)
//
// Then load the ISO from the dashboard's PlayStation 2 screen (BYOD), or any
// PS2/emulator that mounts ISO9660. build.sh --iso passes an explicit outIso.
...
const appName = appDir.replace(/\\/g, "/").replace(/\/+$/, "").split("/").pop() || "athena";
const outIso = Deno.args[1] ?? `${appName}.iso`;
...
// Drop the original ELF entry, any pre-existing SYSTEM.CNF, and any pre-existing
// ATHA_000.01 boot stub — we re-add SYSTEM.CNF + ATHA_000.01 below. (build.sh
// pre-creates ATHA_000.01 in build/ for raw USB boot; skipping it here avoids a
// duplicate ISO record and a wasted ~5 MB copy of the ELF.)
const rootFiles = root.filter(
  (n) => n.name !== elfOrig && n.name.toUpperCase() !== "SYSTEM.CNF" && n.name.toUpperCase() !== "ATHA_000.01",
);
...
// Ensure the output's parent directory exists (Deno.writeFile won't create it).
const outParent = outIso.replace(/[\\/]+[^\\/]*$/, "");
if (outParent && outParent !== outIso) {
